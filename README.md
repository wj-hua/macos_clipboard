...
4. 使用上下方向键选择文本、左右方向键切换标签；按 `Option + 1` 到 `Option + 9` 可直接将对应文本粘贴到当前输入框，按回车粘贴当前选中项，按 `Esc` 关闭。
...
8. 在面板的空白处（标题栏、底栏留白等非按钮区域）按住约 0.4 秒后拖动，即可移动面板位置；按钮、文本列表和文本编辑器保留各自的点击与拖拽行为，标签栏依旧是长按拖动排序。
