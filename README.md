...
4. 面板打开后可直接输入关键词搜索；支持搜索当前标签或全部标签，也可按 `Command + F` 聚焦搜索框。使用上下方向键或纵向滚轮选择文本，使用左右方向键或侧向滚轮切换标签（也支持 `Shift + 滚轮`）；按 `Option + 1` 到 `Option + 9` 可直接将对应搜索结果粘贴到当前输入框，按回车粘贴当前选中项。搜索时按一次 `Esc` 清空关键词，再按一次关闭面板。
